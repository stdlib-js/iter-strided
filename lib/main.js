...
function iterStrided( iterator, stride, offset, eager ) {
	...
	if ( !isIteratorLike( iterator ) ) {
		throw new TypeError( format( '0Px46,G5', iterator ) );
	}
	if ( !isPositiveInteger( stride ) ) {
		throw new TypeError( format( '0Px45,IB', stride ) );
	}
	if ( arguments.length === 3 ) {
		if ( !isNonNegativeInteger( offset ) ) {
			throw new TypeError( format( '0Px2m,L9', offset ) );
		}
		idx = offset;
		bool = DEFAULT_EAGER;
		...
		idx = offset;
		if ( !isBoolean( eager ) ) {
			throw new TypeError( format( '0Px4J,Lr', eager ) );
		}
		bool = eager;
	} else {
		idx = DEFAULT_OFFSET;
