...
function iterStrided( iterator, stride, offset, eager ) {
	...
	if ( !isIteratorLike( iterator ) ) {
		throw new TypeError( format( '0Px46', iterator ) );
	}
	if ( !isPositiveInteger( stride ) ) {
		throw new TypeError( format( '0Px45', stride ) );
	}
	if ( arguments.length === 3 ) {
		if ( !isNonNegativeInteger( offset ) ) {
			throw new TypeError( format( '0Px2m', offset ) );
		}
		idx = offset;
		bool = DEFAULT_EAGER;
		...
		idx = offset;
		if ( !isBoolean( eager ) ) {
			throw new TypeError( format( '0Px4J', eager ) );
		}
		bool = eager;
	} else {
		idx = DEFAULT_OFFSET;
